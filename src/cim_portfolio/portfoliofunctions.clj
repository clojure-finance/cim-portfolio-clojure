...
;; Table Visualization (might refactor as this is generated by AI)

(defn sample-returns [returns-map]
  (let [sorted-dates (sort (keys returns-map))
        n (count sorted-dates)
        step (max 1 (int (/ n 10)))
        sampled-dates (map #(nth sorted-dates %) (range 0 n step))]
    (map (fn [date]
           {"Date" date
            "Return" (get returns-map date)})
         sampled-dates)))

(defn stock-table [ticker data]
  [:div
   [:h3 ticker]
   (kind/table
    {:column-names ["Date" "Return"]
     :row-maps (sample-returns (:arithmetic-returns data))})])

(defn layout-tables [stock-data]
  (kind/hiccup
   [:div {:style {:display "flex" :flex-direction "column" :gap "40px"}}
    (for [row (partition-all 3 (map (fn [[ticker data]] (stock-table ticker data)) stock-data))]

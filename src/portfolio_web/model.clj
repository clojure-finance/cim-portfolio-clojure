...
(defn process-trades [raw-data]
  (let [;; Variables 
        ...
        ;; Raw Variables
        trades (actions/adjust-trades-for-splits (:trades raw-data)) ;; Normalize share amounts and user-supplied prices to post-split units, consistent with Yahoo's split-adjusted prices
        ...
        ;; This variable does not include cash
        ...
        (let [;; Date Variables
              ...
              set-of-portfolio-complete-data (portfolio/set-of-portfolio-log-returns-and-weights portfolio-composition-by-date
                                                                                                 portfolio-value-by-day ;; not a map, it is in [date_str value, ...] format
                                                                                                 (.toString one-year-from-five-weeks-ago)
                                                                                                 (.toString today))
              ;; Complete Log Returns Time-Series from oldest date to today, already in the form of a map of the form {date_str log_return}
              portfolio-log-returns (:portfolio-log-returns set-of-portfolio-complete-data)
...
;; Contains usage of deprecated function, will replace with set-of-portfolio-log-returns-and-weights soon
        ...
        ;; (the mean-return window expands from day two until it reaches 21 trading days, so young portfolios get values immediately)
        ...
        {:x (map #(first %) portfolio-value-by-day)
         :y (cons nil default-rolling-sharpe-ratio) ;; Only the first day has no value — the mean return window expands until it reaches 21 days
         :type "scatter"
         :mode "lines"
        ...
        {:x (map #(first %) portfolio-value-by-day)
         :y (cons nil alternative-rolling-sharpe-ratio) ;; Only the first day has no value — the mean return window expands until it reaches 21 days
         :type "scatter"
         :mode "lines"
